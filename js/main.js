$(document).ready(function(){
    ...
    $(window).on('load resize',function(){
        ...
        // 만약 윈도우의 가로길이가 1199보다 작으면 (태블릿, 모바일)
        // 세로아코디언 메뉴
        if(bodyW <= 1199){
            $('.acc_item').removeClass('active');
            $('.sitemap .site_logo img').attr('src','img/black_logo.png')
        ...
        // 윈도우의 가로길이가 1200보다 크거나 같으면 (pc버전)
        else{
            // 주메뉴에 마우스 오버 이벤트설정
    ...
    // 화면이 스크롤 되어서 첫번째 section이 지나가면 header에
    // active 설정
    $(window).scroll(function(){

        // 윈도우의 scrollTop값을 변수에 저장
        ...
        if(sTop1 < top && top < sTop2){
            $('.happy_txt').addClass('active');
            $('.happy_img').addClass('active');
        }
    ...
    //   swiper에 play/stop 추가
    $('section.s1 .swiper-pagination').append('<span class="swiper-play-stop"></span>');
    // swiper-play-stop 버튼을 클릭할 때마다 배경 이미지도
    // 변경하고 멈췃다가 다시 재생되는 기능
    var sw=0;
    ...
    var swiper2 = new Swiper(".mySwiper2", {
        ...
        loop:true,
        breakpoints:{
            1200:{  //가로길이 1280px 이상 (pc버전)
                slidesPerView:4
            },
            768:{  //가로길이 768px 이상 (테블릿버전)
                slidesPerView:2.1
            },
            360:{ //가로길이 360~1199px 사이 (모바일)
                slidesPerView:1.6,
                scrollbar:{
                    el:".swiper-scrollbar",
                    hide:false
